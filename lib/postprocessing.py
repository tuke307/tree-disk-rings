...
from lib.interpolation_nodes import (
...
from lib.connect_chains import (
...
def from_shapely_to_chain(
    uncompleted_shapely_chain, uncomplete_chain, shapely_inward_chains_subset
):
    inward_chain_subset = [
...
class Ring:
    ...
    def draw(self, image):
        ...
        pts = np.array(lista_pts, np.int32)
...
def select_support_chain(outward_ring, inward_ring, endpoint):
    ...
    distance = [
        ch.euclidean_distance_between_nodes(endpoint, completed_chain_node)
    ...
    if len(distance) < 2:
        ...
        support_chain = ch.get_chain_from_list_by_id(
            chains_in_radial_direction,
...
def exist_angular_intersection_with_src_chain(
    chain: ch.Chain, src_chain_angular_domain: List[int]
):
...
def select_no_intersection_chain_at_endpoint(
    ...
    endpoint = (
        ch.EndPoints.A if ray_direction == src_chain.extA.angle else ch.EndPoints.B
    )
    direction = (
        ch.ClockDirection.clockwise
        if endpoint == ch.EndPoints.B
        else ch.ClockDirection.anti_clockwise
    )
    ...
    domain1 = (
        ch.get_nodes_angles_from_list_nodes(ch1_sub.l_nodes)
        if ch1_sub.size > 0
        else src_nodes
    )
    domain2 = (
        ch.get_nodes_angles_from_list_nodes(ch2_sub.l_nodes)
        if ch2_sub.size > 0
        else src_nodes
    )
...
def split_intersecting_chain_in_other_endpoint(
    endpoint, src_chain, within_chain_set, within_nodes, chain_search_set
):
    """
    ...
    """
    node_other_endpoint = (
        src_chain.extB if endpoint == ch.EndPoints.A else src_chain.extA
    )
...
def filter_no_intersected_chain_far(
    no_intersecting_chains, src_chain, endpoint, neighbourhood_size=45
):
    """
...
def add_chains_that_intersect_in_other_endpoint(
    ...
    for in_chain in within_chain_set:
        if in_chain in no_intersections_chain + search_chain_set:
            continue
        if (
            ch.angular_distance_between_chains(src_chain, in_chain, endpoint)
            < neighbourhood_size
        ):
            endpoint_in_chain = (
                in_chain.extA if ch.EndPoints.A == endpoint else in_chain.extB
            )
...
def get_chains_that_satisfy_similarity_conditions(
    state, support_chain, src_chain, search_chain_set, endpoint
):
    """
    ...
    while True:
        ...
        if check_pass:
            ...
            endpoint_src = (
                src_chain.extA if endpoint == ch.EndPoints.A else src_chain.extB
            )
            ...
            endpoint_distance = ch.euclidean_distance_between_nodes(
...
def select_closest_candidate_chain(
    l_candidate_chains,
    l_candidate_chain_euclidean_distance,
    l_radial_distance_candidate_chains,
    l_within_chains,
    aux_chain,
):
    """
...
def split_and_connect_chains(
    l_within_chains: List[ch.Chain],
    inward_ring: ch.Chain,
    outward_ring: ch.Chain,
    l_ch_p: List[ch.Chain],
    l_nodes_c: List[ch.Node],
...
def connect_2_chain_via_support_chain(
    ...
    inner_candidate_chain = ch.get_chain_from_list_by_id(
    ...
    global_candidate_chain = ch.get_chain_from_list_by_id(
        chain_list, candidate_chain.id
    )
    ...
    for chain in chain_list:
        if chain.is_closed() and chain.size < chain.Nr:
            inward_chain, outward_chain, _ = get_inward_and_outward_visible_chains(
                chain_list, chain, ch.EndPoints.A
            )
