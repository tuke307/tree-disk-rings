...
def saving_results(res, output_dir, save_imgs=True):
    ...
    ch.visualize_chains_over_image(
        img=im_seg, filename=f"{output_dir}/segmentation.png"
    )
    ch.visualize_chains_over_image(
        img=im_pre, filename=f"{output_dir}/preprocessing.png"
    )
    ch.visualize_chains_over_image(
    ...
    ch.visualize_chains_over_image(
        chain_list=[
            ...
            for chain in ch_p
            if chain.is_closed()
            and chain.type not in [ch.TypeChains.center, ch.TypeChains.border]
        ],
        img=im_seg,
