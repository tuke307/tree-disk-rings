...
class InfoVirtualBand:
    ...
    def is_dot_in_band(self, node: ch.Node):
        ...
        outter_band = (
            self.sup_band if inf_mean_radii < sup_mean_radii else self.inf_band
        )
...
def regularity_of_the_derivative_condition(
    state, Nr, ch_jk_nodes, ch_j_nodes, ch_k_nodes, endpoint_j, th_deriv=1
):
    """
    ...
    if derivative_from_center:
        ...
        for node in node_list:
            if node.angle not in angles:
                ...
                new_list.append(
                    ch.get_node_from_list_by_angle(virtual_nodes, node.angle)
                )
...
def generate_virtual_nodes_between_two_chains(
    ...
    domain_interpolation(
        support_chain, cad1_endpoint, cad2_endpoint, endpoint, src_chain, virtual_nodes
    )
...
def radial_tolerance_for_connecting_chains(
    state, th_radial_tolerance, endpoints_radial
):
    """
...
class Neighbourhood:
    ...
    def __init__(self, src_chain, dst_chain, support_chain, endpoint, n_nodes=20):
        ...
        """
        self.virtual_nodes = generate_virtual_nodes_between_two_chains(
            src_chain, dst_chain, support_chain, endpoint
        )
        self.src_endpoint = (
            src_chain.extB if endpoint == ch.EndPoints.B else src_chain.extA
        )
        self.dst_endpoint = (
            dst_chain.extA if endpoint == ch.EndPoints.B else dst_chain.extB
        )
        ...
        self.radial_distance_endpoints_to_support = (
            radial_distance_between_nodes_belonging_to_same_ray(
        ...
        self.src_chain_nodes = (
            src_chain.sort_dots(direction=ch.ClockDirection.anti_clockwise)[:n_nodes]
            if endpoint == ch.EndPoints.A
            else src_chain.sort_dots(direction=ch.ClockDirection.clockwise)[:n_nodes]
        )
        self.dst_chain_nodes = (
            dst_chain.sort_dots(direction=ch.ClockDirection.clockwise)[:n_nodes]
            if endpoint == ch.EndPoints.A
            else dst_chain.sort_dots(direction=ch.ClockDirection.anti_clockwise)[
                :n_nodes
            ]
        )
        self.set_i = radial_distance_between_nodes_belonging_to_same_ray(
...
def similar_radial_distances_of_nodes_in_both_chains(
    state, distribution_th, set_j, set_k
):
    """
...
def exist_chain_in_band_logic(
    chain_list: List[ch.Chain], band_info: InfoVirtualBand
) -> list:
