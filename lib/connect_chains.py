...
from lib.interpolation_nodes import compute_interpolation_domain, domain_interpolation
from lib.basic_properties import (
...
def extract_border_chain_from_list(ch_s: List[ch.Chain], nodes_s: List[ch.Node]):
    ...
    """
    ch_s_without_border = [
        chain for chain in ch_s if chain.type != ch.TypeChains.border
    ]
...
class SystemStatus:
    def __init__(
        ...
        cx,
        Nr=360,
        th_radial_tolerance=0.1,
        th_distribution_size=2,
        th_regular_derivative=1.5,
    ...
    def get_common_chain_to_both_borders(self, chain: ch.Chain):
        ...
        nodes_in_ray_a.sort(
            key=lambda x: ch.euclidean_distance_between_nodes(x, chain.extA)
        )
    ...
    def add_nodes_list_to_system(self, chain, l_nodes: List[ch.Node]):
        ...
        for new_dot in l_nodes:
            ...
            if up_dot is not None:
                up_chain = ch.get_chain_from_list_by_id(
            ...
            if down_dot is not None:
                down_chain = ch.get_chain_from_list_by_id(
...
def connect_chains_main_logic(
    ...
    l_ch_s,
    l_nodes_s,
    th_radial_tolerance=2,
    th_distribution_size=2,
    th_regular_derivative=1.5,
    neighbourhood_size=22,
    derivative_from_center=False,
    debug_imgs=False,
    im_pre=None,
    save=None,
    ...
    while state.continue_in_loop():
        ...
        for location, l_candidates_chi in zip(
            [ch.ChainLocation.inwards, ch.ChainLocation.outwards],
...
def get_closest_chain(
    state: SystemStatus,
    ch_j: ch.Chain,
    l_no_intersection_j: List[ch.Chain],
    ch_i: ch.Chain,
    location: int,
    endpoint: int,
    M,
...
def get_closest_chain_logic(
    state, ch_j, l_candidates_chi, l_no_intersection_j, ch_i, location, endpoint
):
    """
...
def connect_two_chains(
    state: SystemStatus, ch_j, ch_k, l_candidates_chi, endpoint, ch_i
):
    """
...
def get_inward_and_outward_list_chains_via_pointers(
    l_ch_s: List[ch.Chain], support_chain: ch.Chain
):
    """
...
def get_chains_in_neighbourhood(
    ...
    for cand_chain in l_no_intersection_j:
        angular_distance = ch.angular_distance_between_chains(
            ch_j, cand_chain, endpoint
        )
...
def sort_chains_in_neighbourhood(chains_in_neighbourhood: List[Set], ch_j: ch.Chain):
    ...
    for d in unique_angular_distances:
        ...
        euclidean_distance_set = [
            Set(
                ch.minimum_euclidean_distance_between_chains_endpoints(ch_d, ch_j), ch_d
            )
...
def check_endpoints(
    support_chain: ch.Chain, ch_j: ch.Chain, candidate_chain: ch.Chain, endpoint: int
) -> bool:
    """
    ...
    ext_cad_2 = (
        candidate_chain.extB if endpoint == ch.EndPoints.A else candidate_chain.extA
    )
...
def get_inward_and_outward_visible_chains(
    chain_list: List[ch.Chain], chain: ch.Chain, endpoint: str
):
...
def get_dots_in_radial_direction(node_direction: ch.Node, chain_list: List[ch.Chain]):
    chains_in_radial_direction = ch.get_chains_within_angle(
...
def compute_intersection_matrix(
    chains_list: List[ch.Chain], nodes_list: List[ch.Node], Nr: int
):
    """
