...
class DevernayEdges:
    ...
    def __init__(
        ...
        self.image_vec = np.ravel(np.array(self.image, dtype=np.float64)).reshape(
    ...
    def compute_gradient(self):
        for x in range(1, self.img_x - 1):
            for y in range(1, self.img_y - 1):
                self.Gx[x + (y * self.img_x)] = (
                    self.image_vec[(x + 1) + (y * self.img_x)]
                    - self.image_vec[(x - 1) + (y * self.img_x)]
                )
                self.Gy[x + (y * self.img_x)] = (
                    self.image_vec[x + ((y + 1) * self.img_x)]
                    - self.image_vec[x + ((y - 1) * self.img_x)]
                )
                self.modG[x + (y * self.img_x)] = np.sqrt(
                    self.Gx[x + (y * self.img_x)] * self.Gx[x + (y * self.img_x)]
                    + self.Gy[x + (y * self.img_x)] * self.Gy[x + (y * self.img_x)]
                )
    ...
    def chain_edge_points(self):
        for x in range(2, self.img_x - 2):
            for y in range(2, self.img_y - 2):
                if (
                    self.Ex[x + (y * self.img_x)] >= 0.0
                    and self.Ey[x + (y * self.img_x)] >= 0.0
                ):
    ...
    def thresholds_with_hysteresis(self):
        ...
        for i in range(0, self.img_x * self.img_y):
            if (
                (self.prev_[i] >= 0 or self.next_[i] >= 0)
                and (not valid[i])
                and (self.modG[i] >= self.high_thresh)
            ):
                valid[i] = True
    ...
    def chain(self, from_, to_):
        if (
            from_ < 0
            or to_ < 0
            or from_ >= (self.img_x * self.img_y)
            or to_ >= (self.img_x * self.img_y)
        ):
        ...
        if (self.Gy[from_] * dx - self.Gx[from_] * dy) * (
            self.Gy[to_] * dx - self.Gx[to_] * dy
        ) <= 0.0:
            return 0.0

        if (self.Gy[from_] * dx - self.Gx[from_] * dy) >= 0.0:
            return 1.0 / dist(
                self.Ex[from_], self.Ey[from_], self.Ex[to_], self.Ey[to_]
            )
        else:
            return -1.0 / dist(
                self.Ex[from_], self.Ey[from_], self.Ex[to_], self.Ey[to_]
    ...
    def detect_edges(self):
        if self.sigma == 0.0:
            ...
            self.image_filtered = self.image.filter(
                ImageFilter.GaussianBlur(radius=self.sigma)
            )
            self.image_vec = np.ravel(
                np.array(self.image_filtered, dtype=np.float64)
            ).reshape([self.img_x * self.img_y, 1])
            self.compute_gradient()
    ...
    def get_gradient_images(self) -> tuple:
        """
        Returns the gradient images in x and y directions as PIL Images.
        Should be called after detect_edges() or at least after compute_gradient().

        Returns:
            tuple: (gx_normalized, gy_normalized) - PIL Images of the gradient images in x and y directions.
        """
        # Reshape the gradient vectors into 2D arrays
        gx_2d = self.Gx.reshape((self.img_y, self.img_x))
        gy_2d = self.Gy.reshape((self.img_y, self.img_x))
        ...
        # gx_normalized = normalize_to_uint8(gx_2d)
        # gy_normalized = normalize_to_uint8(gy_2d)

        # Convert to PIL Images
        # gradient_x_image = Image.fromarray(gx_normalized)
        # gradient_y_image = Image.fromarray(gy_normalized)
